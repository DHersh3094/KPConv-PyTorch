...
def save_subsampled_clouds(points, batch_lengths, layer_idx, config, labels=None, cloud_names=None):
    
    outdir = os.path.join(config.path, config.save_clouds_path, f'layer_{layer_idx}')
    os.makedirs(outdir, exist_ok=True)
    
    start_idx = 0 # layer 2 of 5
    
    for cloud_idx, length in enumerate(batch_lengths):
        if length > 0:
            cloud_points = points[start_idx:start_idx + length]
            
            if cloud_names is not None and cloud_idx < len(cloud_names):
                filename =  f'{cloud_names[cloud_idx]}_{layer_idx}.ply'
                filepath = os.path.join(outdir, filename)
                if not os.path.exists(filepath): # Only save first time
                    write_ply(filepath, [cloud_points], ['x', 'y', 'z'])
            ...
            if 'pool' in block or 'strided' in block:
                ...
                if hasattr(self.config, 'do_grid_subsample') and self.config.do_grid_subsample:
                    ...
                    if hasattr(self, 'mode') and self.mode == 'train':
                        ...
                        save_subsampled_clouds(points=pool_p, batch_lengths=pool_b, layer_idx=len(input_points)+1, config=self.config, labels=labels,
